...
def create_app(settings: WebSettings | None = None) -> FastAPI:
    ...
    async def operation_error(request: Request, error: WebOperationError) -> JSONResponse:
        return _error(request, error.status_code, error.code, error.safe_message)

    @app.exception_handler(WebQueryNotFoundError)
    async def not_found(request: Request, error: WebQueryNotFoundError) -> JSONResponse:
        del error
    ...
    def query_service(request: Request) -> WebQueryService:
        database = cast(Database | None, getattr(request.app.state, "database", None))
        if database is None:
            raise WebOperationError(
                503, "DATABASE_UNAVAILABLE", "The local database is unavailable"
            )
    ...
    def action_service(request: Request) -> WebActionService:
    ...
    def messages(
        service: Annotated[WebQueryService, Depends(query_service)],
    ...
    def actions(
        service: Annotated[WebActionService, Depends(action_service)],
